        ...
        <div data-app>
            <v-app-bar dense flat color="rgba(0, 0, 0, 0)">
                ...
                <v-menu bottom left>
                    ...
                    <v-list>
                        <v-list-item>
                            <v-btn elevation="2" x-small color="primary" rounded v-on:click="deleteServer" v-if="is_admin && !ro"><i class="fas fa-trash"></i>Del</v-btn>
                        </v-list-item>
                        <v-divider></v-divider>
                        <v-list-item>
                            <v-btn elevation="2" x-small color="primary" rounded v-on:click="edit" v-if="is_admin && !ro"><i class="fas fa-cog"></i>Settings</v-btn>
                        </v-list-item>
                        <v-divider></v-divider>
                        <v-list-item>
                            <v-btn elevation="2" x-small color="primary" rounded v-on:click="logs" v-if="is_admin && !ro"><i class="fas fa-terminal"></i>Logs</v-btn>
                        </v-list-item>
                        <v-divider></v-divider>
                        <v-list-item>
                            <v-btn elevation="2" x-small color="primary" rounded v-on:click="copyConfig" v-if="is_admin && !ro"><i class="fas fa-copy"></i>Copy CFG</v-btn>
                        </v-list-item>
                        <v-divider></v-divider>
                        <v-list-item>
                            <v-btn elevation="2" x-small color="primary" rounded v-on:click="exportConfig" v-if="is_admin && !ro"><i class="fas fa-file-download"></i>Download CFG</v-btn>
                        </v-list-item>
                    </v-list>
                </v-menu>
            </v-app-bar>
            <v-list-item active-class>
                <v-list-item-content>
                    ...
                    <div class="info state" v-if="server.pid">
                        ...
                        <v-divider></v-divider>
                        <b>{{ $t("session") }}: </b>
                        <span v-if="server.sessionType">{{ server.sessionType }} ({{ server.sessionPhase }}) -
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-row dense>
                    <v-btn elevation="2" x-small color="primary" rounded class="start" v-on:click="start" v-if="is_mod && !ro && !server.pid"><i class="fas fa-play"></i>{{ $t("start_server") }}</v-btn>
                    <v-btn elevation="2" x-small color="primary" rounded class="stop" v-on:click="stop" v-if="is_mod && !ro && server.pid"><i class="fas fa-stop"></i>{{ $t("stop_server") }}</v-btn>

                    <v-btn elevation="2" x-small color="primary" rounded v-on:click="live" v-if="is_mod && !ro && server.pid"><i class="fas fa-tv"></i>{{ $t("view_live") }}</v-btn>
