				...
				<v-row dense>
					<v-col
						><v-btn
							...
							x-small
							class="primary"
							v-on:click="$router.push('/server')"
							v-if="is_admin"
							><i class="fas fa-plus"></i> {{ $t("add_new") }}</v-btn
						></v-col
					>
					<v-col
						><v-btn
							elevation="2"
							x-small
							v-bind:class="stopAllClass"
							v-on:click="stopAllServers"
							v-if="is_mod || is_admin"
							><i class="fas fa-stop"></i> {{ $t("stop_all") }}</v-btn
						></v-col
					>
					<v-col
						><v-btn elevation="2" x-small v-on:click="loadServer(true)"
							><i class="fas fa-sync"></i> {{ $t("refresh") }}</v-btn
						></v-col
					>
					<v-col
						><v-btn
							...
							class="logout-btn"
							v-on:click="logout"
							v-bind:title="$t('log_out')"
							><i class="fas fa-sign-out-alt"></i></v-btn
					></v-col>
				</v-row>
			</div>
		</div>
